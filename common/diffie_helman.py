...
class DiffieHelman:
    ...
    def _new_authentication_key(self):
        ...
        cursor.executemany(
            "INSERT INTO ed448_authentication_key (key, type) VALUES (?, ?)",
            [(privkey_pem, "PRIVATE"), (pubkey_pem, "PUBLIC")]
        )
        self.conn_.commit()
    ...
    def _new_identity_key(self):
        ...
        cursor.executemany(
            "INSERT INTO x448_identity_key (key, type) VALUES (?, ?)",
            [(privkey_pem, "PRIVATE"), (pubkey_pem, "PUBLIC"), (signature, "SIGNATURE")]
        )
        self.conn_.commit()
    ...
    def check_password(self):
        ...
        cursor = self.conn_.cursor()
        cursor.execute("SELECT SHA256 FROM password LIMIT 1;")
        result = cursor.fetchone()

        if result is None:
            cursor.execute("INSERT INTO password (SHA256) VALUES (?);", (password_hash,))
            self.conn_.commit()
        else:
    ...
    def ensure_master_keys(self):
        ...
        authentication_key = cursor.fetchall()

        if authentication_key == []:
            self._new_authentication_key()
            self._new_identity_key()

    def signed_pre_keys_pub(self) -> str:
        cursor = self.conn_.cursor()
        cursor.execute(
            "SELECT id, key, type, nvb, nva FROM x448_signed_pre_keys WHERE type IN (?, ?)",
            ("PUBLIC", "SIGNATURE")
        )
        ...
        return base64.b64encode(msgpack.dumps(keys)).decode()
    ...
    def signed_pre_key_prv(self, timestamp: datetime):
        ...
        for id, nvb, nva in data:
            if datetime.fromisoformat(nvb).replace(tzinfo=timezone.utc) <= timestamp <= datetime.fromisoformat(
                ...
                cursor.execute(
                    "SELECT key FROM x448_signed_pre_keys WHERE id = ? AND type = ?",
                    (id, "PRIVATE",)
                )
                result = cursor.fetchone()
    ...
    def remove_old_signed_pre_keys(self):
        ...
        cursor = self.conn_.cursor()
        cursor.execute("DELETE FROM x448_signed_pre_keys WHERE nvb < ?", (threshold_date,))
        self.conn_.commit()
    ...
    def renew_signed_pre_keys(self):
        ...
        if last_nvb < now + timedelta(days=30):
            while last_nvb < now + timedelta(days=50):
                ...
                cursor.executemany(
                    'INSERT INTO x448_signed_pre_keys (id, key, type, nvb, nva) VALUES (?, ?, ?, ?, ?)',
                    [
